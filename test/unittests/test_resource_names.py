...
A file whose basename does not match the name in ``@intent_handler`` or
``speak_dialog`` is never found: the intent silently fails to register and the
dialog silently falls back to en-US, so the locale looks translated while the
user hears English.
"""
import re
